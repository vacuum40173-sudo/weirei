...
<head>
    ...
    <style>
        ...
        .btn-checkout { background-color: var(--secondary); }
...
<div class="card no-print">
    ...
    <div style="background: #fcfcfc; border: 1px solid #eee; padding: 12px; border-radius: 8px;">
        <label>👤 客戶基本資料</label>
        <div class="row"><input type="text" id="cust_name" placeholder="姓名"><input type="tel" id="cust_phone" placeholder="電話"></div>
    </div>
</div>
...
<div class="card no-print" id="inputSection">
    ...
    <div class="section">
        ...
        <select id="mode" onchange="toggleMode()">
            <option value="normal" selected>國畫裝框 (周長計價)</option>
            <option value="oil">油畫裝框 (堆疊計價)</option>
            <option value="board">國畫裱板 (面積計價)</option>
            <option value="scroll">立軸掛軸</option>
        </select>
    </div>

    <div class="row" style="margin-top:10px;">
        <div style="flex:1;"><label>畫心長 (三位數)</label><input type="number" id="L_in" placeholder="000" oninput="runCalc()" inputmode="numeric"></div>
        <div style="flex:1;"><label>畫心寬 (三位數)</label><input type="number" id="W_in" placeholder="000" oninput="runCalc()" inputmode="numeric"></div>
    </div>
    <div class="row">
        <div style="flex:1;"><label>預留長邊 (尺)</label><input type="number" id="padL" value="1.0" step="0.1" oninput="runCalc()"></div>
        <div style="flex:1;"><label>預留短邊 (尺)</label><input type="number" id="padW" value="0.5" step="0.1" oninput="runCalc()"></div>
    </div>

    <div id="normal-options" class="option-group">
        <label>選擇框種與基數</label>
        <select id="baseRate" onchange="runCalc()">
            <option value="250">國畫框 (250)</option>
            <option value="280">2寸特選手工框 (280)</option>
            <option value="300">3寸特選手工框 (300)</option>
            <option value="inner_outer">國畫內外框 (250 - 含壓克力)</option>
        </select>
        <div id="acyRow" style="margin-top:10px;">
            <label style="font-weight:normal;"><input type="checkbox" id="addAcy" onchange="runCalc()" style="width:auto;"> 加壓克力 (120/才)</label>
        </div>
    </div>
    ...
    <div id="oil-options" class="option-group hidden">
        ...
        <select id="oilType" onchange="runCalc()">
            <option value="direct">直接框 (單純框條)</option>
            <option value="solid">立體框 (底板+框條)</option>
            <option value="inout">內外框 (底板+框條+內框)</option>
        </select>
        <label style="margin-top:10px;">外框條基數</label>
        <select id="oilFrameBase" onchange="runCalc()">
            <option value="20">6分框 (20)</option>
            <option value="25">寸半框 (25)</option>
            <option value="35">兩寸半框 (35)</option>
            <option value="45">三寸框 (45)</option>
        </select>
    </div>

    <div class="section"><label>📝 單項備註</label><input type="text" id="item_note" placeholder="如：加金邊、特殊顏色..."></div>
    ...
    <button class="btn btn-add" onclick="addItem()">＋ 加入報價清單</button>
</div>
...
<div class="card" id="cartSection">
    ...
    <div style="text-align:right; font-size:1.3rem; font-weight:bold; color:#c0392b; margin-top:15px; border-top:2px solid #eee; padding-top:10px;">
        總計：NT$ <span id="totalTxt">0</span>
    </div>
    <div class="no-print">
        <button class="btn" style="background:#3498db;" onclick="window.print()">列印單據 / 截圖存檔</button>
        <button class="btn" style="background:#95a5a6; margin-top:10px;" onclick="location.reload()">清空重新輸入</button>
    </div>
</div>
...
<script>
    ...
    function runCalc() {
        ...
        // 取得畫心尺寸並轉換為「台尺」 (輸入 123 代表 1.23 尺)
        const L = (parseFloat(document.getElementById('L_in').value) || 0) / 100;
        const W = (parseFloat(document.getElementById('W_in').value) || 0) / 100;
        
        // 預留邊 (這部分維持手動輸入尺)
        const pL = parseFloat(document.getElementById('padL').value) || 0;
        const pW = parseFloat(document.getElementById('padW').value) || 0;
        ...
        const cai = Math.ceil((Linch * Winch) / 144); // 壓克力採無條件進位才數
        
        document.getElementById('sizeDetail').innerText = `框內尺寸：${Lcm.toFixed(1)} x ${Wcm.toFixed(1)} cm (${tL.toFixed(2)} x ${tW.toFixed(2)} 尺)`;
        ...
        if (mode === 'normal') {
            ...
            // 油畫計算：底板費用(50/才) + 外框費用(周長*基數) + 內框(12/周長)
            ...
            currentPrice = 350; // 裱板/立軸預設低消，待詳細公式
        }
    ...
    function renderCart() {
        ...
        cart.forEach((it, idx) => {
            ...
            table.innerHTML += `<tr>
                ...
                <td align="right">
                    <button onclick="cart.splice(${idx},1);renderCart()" style="color:red;border:none;background:none;font-size:1.2rem;cursor:pointer;">×</button>
                </td>
            </tr>`;
        });
